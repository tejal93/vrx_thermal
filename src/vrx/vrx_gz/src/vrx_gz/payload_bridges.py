...
def image(world_name, model_name, link_name, sensor_name):
    ...
    print("Thermal_camera gz topic",gz_sensor_prefix)
    print("Ros image sensor prfix",ros_sensor_prefix)
    return Bridge(
...
def Thermal_image(world_name, model_name, link_name, sensor_name):
    gz_sensor_prefix = gz_prefix(world_name, model_name, link_name, sensor_name)
    print("Thermal_camera gz topic",gz_sensor_prefix)
    ros_sensor_prefix = ros_prefix(sensor_name, 'Thermal_cameras')
    print("Ros Thermal sensor prfix",ros_sensor_prefix)
    return Bridge(
        gz_topic=f'{gz_sensor_prefix}/image',
        ros_topic=f'{ros_sensor_prefix}/image_raw',
...
def payload_bridges(world_name, model_name, link_name, sensor_name, sensor_type):
    ...
    if sensor_type == sdf.Sensortype.THERMAL_CAMERA:
        print("True_thermal "*50)
        bridges = [
            Thermal_image(world_name, model_name, link_name, sensor_name),
            camera_info(world_name, model_name, link_name, sensor_name)
        ]
    elif sensor_type == sdf.Sensortype.CAMERA:
        print("False_thermal"*50)
        bridges = [
